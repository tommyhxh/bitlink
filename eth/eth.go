...
func InitEthClient() {
	client, err := ethclient.Dial("http://10.10.10.203:8545")
	if err != nil {
...
func logTran(tx *types.Transaction, msg types.Message, block *types.Block) {
...
func GetTransaction(blockIdx uint64, addrMap map[common.Address]bool) []entity.TxMsg {
	...
	for start {
		//通过区块高度获取块信息
		block, err := EthClient.BlockByNumber(context.Background(), big.NewInt(int64(blockIdx)))
		if err != nil {
			fmt.Println(err.Error())
...
func Eth() {
	...
	for blockIdx <= 626566 {
		...
		//例如，区块号，区块时间戳，区块摘要，区块难度以及交易列表等等
		// context.Background() 返回一个空的Context 我们可以用这个
		//空的 Context 作为 goroutine 的root 节点
		...
		for k, tx := range block.Body().Transactions {
			// 为了读取发送方的地址，我们需要在事务上调用AsMessage，
			//它返回一个Message类型，其中包含一个返回sender（from）地址的函数。
